...
async def test_add_role():
    async with async_session_maker() as session:
        ...
        assert ([i.role for i in result.scalars()]) == ['ROLE_PORTAL_ADMIN', 'ROLE_PORTAL_USER']
...
async def test_register(client):
    async with async_session_maker() as session:
        ...
        await session.execute(stmt)
        await session.commit()
    response = await client.post("user/users", json={
        "nickname": "string",
        "password": "string"
    })
    ...
    assert user.nickname == result.get('nickname')
    ...
    assert result.get('nickname') == 'string'
